...
let amourBélier = signeTrouve.amour;
let argentBélier = signeTrouve.argent;
let santeBélier = signeTrouve.sante;
let familleBélier = signeTrouve.famille;
let conseilBélier = signeTrouve.conseil;
let imageBélier = signeTrouve.image;
let signeBélier = signeTrouve.signe;
let dateBélier = signeTrouve.date;

// recuperer les données du signe Cancer
let travailCancer = signeTrouve.travail;
let amourCancer = signeTrouve.amour;
let argentCancer = signeTrouve.argent;
let santeCancer = signeTrouve.sante;
let familleCancer = signeTrouve.famille;
let conseilCancer = signeTrouve.conseil;
let imageCancer = signeTrouve.image;
let signeCancer = signeTrouve.signe;
let dateCancer = signeTrouve.date;

// recuperer les données du signe Capricorne
let travailCapricorne = signeTrouve.travail;
let amourCapricorne = signeTrouve.amour;
let argentCapricorne = signeTrouve.argent;
let santeCapricorne = signeTrouve.sante;
let familleCapricorne = signeTrouve.famille;
let conseilCapricorne = signeTrouve.conseil;
let imageCapricorne = signeTrouve.image;
let signeCapricorne = signeTrouve.signe;
let dateCapricorne = signeTrouve.date;

// recuperer les données du signe Gémeaux
let travailGémeaux = signeTrouve.travail;
let amourGémeaux = signeTrouve.amour;
let argentGémeaux = signeTrouve.argent;
let santeGémeaux = signeTrouve.sante;
let familleGémeaux = signeTrouve.famille;
let conseilGémeaux = signeTrouve.conseil;
let imageGémeaux = signeTrouve.image;
let signeGémeaux = signeTrouve.signe;
let dateGémeaux = signeTrouve.date;

// recuperer les données du signe Lion
let travailLion = signeTrouve.travail;
let amourLion = signeTrouve.amour;
let argentLion = signeTrouve.argent;
let santeLion = signeTrouve.sante;
let familleLion = signeTrouve.famille;
let conseilLion = signeTrouve.conseil;
let imageLion = signeTrouve.image;
let dateLion = signeTrouve.date;
let signeLion = signeTrouve.signe;



let amour_signe = document.getElementById('amour');
const amour=document.querySelector("#amour")
amour.insertAdjacentHTML('beforeend', signeTrouve.amour);

let travail_signe = document.getElementById('travail');
const travail=document.querySelector("#travail")
travail.insertAdjacentHTML('beforeend', signeTrouve.travail);

let argent_signe = document.getElementById('argent');
const argent=document.querySelector("#argent")
argent.insertAdjacentHTML('beforeend', signeTrouve.argent);

let sante_signe = document.getElementById('sante');
const sante=document.querySelector("#sante")
sante.insertAdjacentHTML('beforeend', signeTrouve.sante);

let famille_signe = document.getElementById('famille');
const famille=document.querySelector("#famille")
famille.insertAdjacentHTML('beforeend', signeTrouve.famille);

let conseil_signe = document.getElementById('conseil');
const conseil=document.querySelector("#conseil")
conseil.insertAdjacentHTML('beforeend', signeTrouve.conseil);

let date_signe = document.getElementById('date');
...
//   const html = <p id="amour"><span>Amour :</span>${signeTrouve.amour}</p>
...
//   document.querySelector("article div ").innerHTML=html
//   document.querySelector("h1").innerText="BELIER"
...
// Ajouter un gestionnaire d'événement pour la flèche droite
// arrowRight.addEventListener('click', () => {
//   // Incrémenter l'index actuel
//   currentIndex++;

//   // Vérifier si l'index dépasse la limite supérieure
//   if (currentIndex >= horoscopes.length) {
//     currentIndex = 0; // Revenir au premier signe
//   }

//   // Appeler la fonction pour afficher l'horoscope correspondant à l'index
//   afficherHoroscope(currentIndex);
// });

// // Ajouter un gestionnaire d'événement pour la flèche gauche
// arrowLeft.addEventListener('click', () => {
...
//   if (currentIndex < 0) {
//     currentIndex = horoscopes.length - 1; // Aller au dernier signe
//   }
